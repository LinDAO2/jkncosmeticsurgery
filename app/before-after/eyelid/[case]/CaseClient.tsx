...
import type { EyelidCase } from '@/lib/eyelidCases'

type LightboxState = { srcs: string[]; index: number } | null

export default function CaseClient({ data }: { data: EyelidCase }) {
  ...
  const images = Array.from({ length: data.imageCount }, (_, i) =>
    `/ba/eyelid/${data.slug}/${String(i + 1).padStart(2, '0')}.jpeg`
  )
  ...
  return (
    <>
      ...
      <section className="case-detail-section">
        ...
        <a className="case-detail-back" href="/before-after?category=bleph">← Before and After</a>

        <div className="case-detail-header">
          <span className="section-label">Patient Case</span>
          <h1 className="case-detail-title">{data.procedures[0]}</h1>
        </div>
